import { Coffee, GitFork, Rocket, Sparkles, Wand2, Rss } from 'lucide-react';
  ...
  return (
    <footer className="mt-12 border-t border-border bg-github-canvas-subtle/50 no-page-gutter">
      <div className="container mx-auto px-4 py-6 flex flex-col items-center justify-center gap-3">
        { /* Base-aware RSS href so it works under subpaths */ }
        {/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
        {/* @ts-ignore - import.meta.env is provided by Vite */}
        {(() => { /* IIFE to keep JSX clean */
          ...
          return (
            <>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 text-xs sm:text-sm">
                ...
                {/* New: RSS note pointing to Watch behavior */}
                <a
                  href={rssHref}
                  target="_blank"
                  rel="noreferrer noopener"
                  className="inline-flex items-center gap-1.5 rounded-full border border-border bg-muted/20 px-2.5 py-1 hover:bg-muted/30 transition-colors"
                  aria-label="Preview RSS feed (Watch button opens this too)"
                  title="Tip: The Watch button opens a preview of the RSS feed"
                >
                  <Rss className="h-3.5 w-3.5" aria-hidden />
                  <span>Tip: Watch opens RSS preview</span>
                </a>
              </div>
            </>
          );
