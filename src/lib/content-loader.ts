...
export async function loadContent<T extends BaseContent>(
  ...
  let items: ContentItem<T>[] = typeModules.map(
  ...
  // Remove hidden-tagged items by default
  items = items.filter((it) => !(((it as any).tags || []).some((t: string) => t.toLowerCase() === 'hidden')));
...
function applySearchFilters<T extends BaseContent>(
  ...
  filtered = [...filtered].sort((a, b) => {
    if (sort === "title-asc" || sort === "title-desc") {
      const ta = ((a as any).title || (a as any).name || "")
        .toString()
        .toLowerCase();
      const tb = ((b as any).title || (b as any).name || "")
        .toString()
        .toLowerCase();
...
export async function getContentTags(
  ...
  const allTags = result.items.flatMap((item) => ((item as any).tags || []));
...
export async function getContentBySlug<T extends BaseContent>(
  ...
  const result = await loadContent<T>(
    contentType,
    {},
    { page: 1, limit: 1000 }
  );
  return (result.items as any[]).find((item) => item.slug === slug) || null;
}
